...
def kennard_stone_sampling(
    ...
    if centrality_measure == "mean":
        ...
        distance_to_center = cdist(
            X, np.median(X, axis=0).reshape(1, -1), metric=metric
        )
    elif centrality_measure == "random":
        distance_to_center = cdist(
            X, X[np.random.choice(X.shape[0])].reshape(1, -1), metric=metric
        )
    else:
    ...
    with tqdm(total=len(X) - 1) as pbar:
        while len(index_remaining):
            ...
            min_distance_to_samples_selected = np.min(
                cdist(samples_selected, X, metric=metric)
            )
...
def pca_kmeans(
    ...
    if n_pca_components is not None:
        ...
        X_pca = X  # noqa: N806
    kmeans = KMeans(
        n_clusters=n_clusters, random_state=random_state, **(kmeans_kwargs or {})
    )
...
def check_fraction(
    train_fraction: float, valid_fraction: float, test_fraction: float
) -> None:
