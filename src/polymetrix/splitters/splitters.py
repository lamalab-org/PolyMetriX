...
from polymetrix.splitters.utils import (
    check_fraction,
    downsample_splits,
...
class BaseSplitter:
    ...
    def __init__(
        ...
        Args:
            ds (AbstractDataset): Dataset to split.
    ...
    def train_test_split(
        ...
        check_fraction(
            train_fraction=frac_train, valid_fraction=0, test_fraction=1 - frac_train
        )
        ...
        if groups is not None:
            ...
            stratification_col = (
                stratification_col[idx] if stratification_col is not None else None
            )
        ...
        if self._sample_frac < 1:
            return sort_arrays_by_len(
                downsample_splits([train_idx, test_index], self._sample_frac),
                self._sort_by_len,
            )
    ...
    def train_valid_test_split(
        ...
        if groups is not None:
            if stratification_col is not None:
                logger.debug("Using grouped stratified partition")
                train_idx, valid_idx, test_index = (
                    grouped_stratified_train_test_partition(
                        stratification_col[idx],
                        groups[idx],
                        frac_train,
        ...
        if self._sample_frac < 1:
            return sort_arrays_by_len(
                downsample_splits(
                    [train_idx, valid_idx, test_index], self._sample_frac
                ),
                self._sort_by_len,
            )
    ...
    def k_fold(self, k: int = 5) -> tuple[Collection[int], Collection[int]]:
        ...
        if groups is not None:
            for train_index, test_index in kfold.split(
                idx, y=stratification_col, groups=groups
            ):
                if self._shuffle:
                    np.random.shuffle(train_index)
                    np.random.shuffle(test_index)
                if self._sample_frac < 1:
                    yield downsample_splits(
                        [train_index, test_index], self._sample_frac
                    )
                else:
...
class TgSplitter(BaseSplitter):
    """Splitter that uses the glass transition temperature to split the dataset.
    For this, we sort structures according to their glass transition temperature and then group the based on the temperature.
    You can modify the number of groups using the :attr:`tg_q` parameter, those values indicate the quantiles which we use for the grouping.
    This ensures that the validation is quite stringent as the different folds will have different glass transition temperatures.
    """
    ...
    def __init__(
        ...
        Args:
            ds (AbstractDataset): A structure dataset.
            group_col (str): The column name of the group column.
            tg_q (Collection[float], optional): List of quantiles used for quantile binning for the tg.
    ...
    def _get_groups(self) -> Collection[int]:
        return quantile_binning(self._ds._df[self._group_col].values, self._grouping_q)
...
class KennardStoneSplitter(BaseSplitter):
    ...
    def get_sorted_indices(self, ds: AbstractDataset) -> Collection[int]:
    ...
    def train_valid_test_split(
        ...
        if self._shuffle:
            return (
                ...
                np.random.permutation(
                    self.get_sorted_indices(self._ds)[
                        num_train_points + num_valid_points :
                    ]
                ),
            )
        return (
...
class ClusterSplitter(BaseSplitter):
    ...
    def _get_sorted_indices(
        self, ds: AbstractDataset, shuffle: bool = True
    ) -> Collection[int]:
        if self._sorted_indices is None:
            feats = ds._df[self.feature_names].values
