...
from polymetrix.core.utils import calculate_metrics, get_fp_ecfp_bitvector
...
def interpolation_split_and_save_data(data, cfg):
    ...
    train_data, test_data = train_test_split(
        data, test_size=cfg.test_size, random_state=cfg.random_state
    )
    train_data, val_data = train_test_split(
        train_data, test_size=cfg.val_size, random_state=cfg.random_state
    )
...
def objective(trial, X_train, y_train, X_val, y_val, model_name, cfg):
    ...
    if model_name == "random_forest":
        ...
        min_samples_split = trial.suggest_int(
            "min_samples_split",
            cfg.random_forest.min_samples_split[0],
            cfg.random_forest.min_samples_split[1],
        )
        min_samples_leaf = trial.suggest_int(
            "min_samples_leaf",
            cfg.random_forest.min_samples_leaf[0],
            cfg.random_forest.min_samples_leaf[1],
        )
        ...
        num_leaves = trial.suggest_int(
            "num_leaves", cfg.lightgbm.num_leaves[0], cfg.lightgbm.num_leaves[1]
        )
        ...
        n_estimators = trial.suggest_int(
            "n_estimators", cfg.xgboost.n_estimators[0], cfg.xgboost.n_estimators[1]
        )
        max_depth = trial.suggest_int(
            "max_depth", cfg.xgboost.max_depth[0], cfg.xgboost.max_depth[1]
        )
        learning_rate = trial.suggest_float(
            "learning_rate",
...
def train_and_evaluate_model(
    X_train, y_train, X_val, y_val, X_test, y_test, model_name, cfg, max_evals=50
):
    """
    ...
    study.optimize(
        lambda trial: objective(trial, X_train, y_train, X_val, y_val, model_name, cfg),
        n_trials=max_evals,
    )
...
def save_results(
    ...
    """
    with open(
        Path(output_dir, f"{model_name}_metrics_{feature_type}_seed_{seed}.json"), "w"
    ) as json_file:
        json.dump(metrics, json_file, indent=4)

    train_predictions.to_csv(
        Path(
            output_dir, f"{model_name}_train_predictions_{feature_type}_seed_{seed}.csv"
        ),
        index=False,
    )
    val_predictions.to_csv(
        Path(
            output_dir, f"{model_name}_val_predictions_{feature_type}_seed_{seed}.csv"
        ),
        index=False,
    )
    test_predictions.to_csv(
        Path(
            output_dir, f"{model_name}_test_predictions_{feature_type}_seed_{seed}.csv"
        ),
        index=False,
    )

    with open(
        Path(output_dir, f"{model_name}_best_params_{feature_type}_seed_{seed}.json"),
        "w",
    ) as json_file:
...
def aggregate_final_metrics(output_dir, model_name, feature_type):
    ...
    with open(
        Path(
            output_dir,
            f"{model_name}_metrics_{feature_type}_final_aggregated_clusters.json",
        ),
        "w",
    ) as json_file:
        ...
        if cfg.split_type == "extrapolation":
            ...
            for cluster in clusters:
                ...
                train_data, val_data, test_data = load_split_data(
                    cfg.split_type, output_dir, cluster, cluster_number
                )
                ...
                for seed in cfg.seeds:
                    ...
                    save_results(
                        output_dir,
                        f"{cfg.feature_type}_cluster_{cluster}",
                        cfg.model_name,
                        metrics,
                        train_predictions,
                        val_predictions,
                ...
                with open(
                    Path(
                        output_dir,
                        f"{cfg.model_name}_metrics_{cfg.feature_type}_cluster_{cluster}_aggregated.json",
                    ),
                    "w",
                ) as json_file:
                    json.dump(aggregated_metrics, json_file, indent=4)
                print(
                    f"Aggregated metrics saved in {Path(output_dir, f'{cfg.model_name}_metrics_{cfg.feature_type}_cluster_{cluster}_aggregated.json')}"
                )

            final_aggregated_metrics = aggregate_final_metrics(
                output_dir, cfg.model_name, cfg.feature_type
            )
            ...
            for seed in cfg.seeds:
                print(f"Running with seed {seed}")
                np.random.seed(seed)
                model, y_train_pred, y_val_pred, y_test_pred, metrics, best_params = (
                    train_and_evaluate_model(
                        X_train,
                        y_train,
                        X_val,
            ...
            with open(
                Path(
                    output_dir,
                    f"{cfg.model_name}_metrics_{cfg.feature_type}_aggregated.json",
                ),
                "w",
            ) as json_file:
                json.dump(aggregated_metrics, json_file, indent=4)
            ...
            print(
                f"Aggregated metrics saved in {Path(output_dir, f'{cfg.model_name}_metrics_{cfg.feature_type}_aggregated.json')}"
            )
